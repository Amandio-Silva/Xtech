...
-- Insert admin user (password: admin123) BEFORE enabling RLS
INSERT INTO users (email, password, name) VALUES 
('admin@xtech.com', 'admin123', 'Admin Xtech')
ON CONFLICT (email) DO NOTHING;

-- Insert sample products BEFORE enabling RLS
...
-- Insert sample services BEFORE enabling RLS
...
-- Create policies for public read access to products and services
...
CREATE POLICY "Anyone can read their own service requests" ON service_requests FOR SELECT USING (true);

-- Create policies for authenticated operations (you'll need to implement proper auth later)
...
CREATE POLICY "Allow all operations on users for now" ON users FOR ALL USING (true) WITH CHECK (true);
